...
const tagIt = (json, path) => {
  ...
  const attributes = JSON.parse(get(json, path.slice(0, path.length - 3).join('.').concat('.attributes'), false))
  return (tag_check === 'a' || (attributes.hasOwnProperty('classList') && attributes.classList.includes('label'))) ?
    false : true
}
