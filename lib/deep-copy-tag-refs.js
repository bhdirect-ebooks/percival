...
const tagIt = (json, path) => {
  ...
  const in_parse =
    ...
    attributes.dataset.hasOwnProperty('context') &&
    JSON.parse(attributes.dataset.context).hasOwnProperty('parsing')
  const no_parse = in_parse
    ? JSON.parse(attributes.dataset.context).parsing === ''
    : false
  const in_label =
  ...
  const in_footnote =
    attributes.hasOwnProperty('className') &&
    attributes.className.includes('note')
  return !(tag_check === 'a' || no_parse || in_label || in_bilio || in_footnote)
}
