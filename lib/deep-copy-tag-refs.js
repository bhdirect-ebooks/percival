...
const tagIt = (json, path) => {
  ...
  const in_parse = attributes.hasOwnProperty('dataset') && attributes.dataset.hasOwnProperty('crossContext') && JSON.parse(attributes.dataset.crossContext).hasOwnProperty('parsing')
  const no_parse = in_parse ? JSON.parse(attributes.dataset.crossContext).parsing === '' : false
  ...
  for (const {parent, node, key, path} of new RecursiveIterator(json, 0, true)) {
    ...
    if (is_text && tagIt(json, path)) {
      ...
      if (type === 'context' && parent.hasOwnProperty('crossContext')) {
        let cross_context
        try {
          cross_context = JSON.parse(parent.crossContext)
        } catch (err) {
          throw new Error(`${parent.crossContext} is not a valid context value.`)
        }
