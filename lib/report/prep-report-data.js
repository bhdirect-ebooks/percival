...
const getRefs = (block_id, orig_html) => {
  ...
  $('a')
    ...
    .each((i, elem) => {
      ...
      if (
        /epub:type="footnote"/.test(orig_html) &&
        data.message !== 'No context.'
      ) {
  ...
  if (page_count <= MAX_PAGES && note_count <= MAX_NOTES) {
    ...
    const chunks = block_html.reduce(
      (a, b) => {
        ...
        if (
          a.page_count < MAX_PAGES &&
          a.note_count < MAX_NOTES &&
          a.page_count + b_page_count < MAX_PAGES &&
          a.note_count + b_note_count < MAX_NOTES
        ) {
          const block_count =
