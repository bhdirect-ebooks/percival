...
const normHtml = html => {
  ...
  return correctXHTML(temp)
    .replace(/(<span[^>]+)\/>/gi, '$1></span>')
    .replace(/(href=")([^\/"#.]+\.xhtml#[^"]+")/g, '$1epub/$2 target="_blank"')
    .replace(/<\/a>(\w)/g, '</a> $1')
}
...
const getRefs = (block_id, orig_html) => {
  ...
  $('a')
    ...
    .each((i, elem) => {
      ...
      // all refs that fall within footnotes need to be unconfirmed so the user must review them
      // ('No context' refs lack "confirmed" and "confidence" attributes, so no need to set them)
      if (/epub:type="footnote"/.test(orig_html) && data.message !== 'No context.') {
        data.confirmed = false
        data.confidence = 9
      }
      $(elem).attr('data-ref', data.scripture)
      const ref_id = `${block_id}-${pad(i + 1, 4)}`
      $(elem).attr('id', ref_id)
...
const chunkify = ({ id, name, final_html }) => {
  const MAX_PAGES = 15
  const MAX_NOTES = 30
  const notes = final_html.match(/<div[^>\n]*epub:type="footnote"[^>\n]*>\s*<p>.*?<\/p>\s*<\/div>/g)
  const note_count = notes ? notes.length : 0
  const pages = final_html.match(/<span[^>\n]+epub:type="pagebreak"[^>\n]+>/g)
  const page_count = pages ? pages.length : 0

  if (page_count <= MAX_PAGES && note_count <= MAX_NOTES) {
    ...
    const chunks = block_html.reduce(
      (a, b) => {
        const notes = b.match(/<div[^>\n]*epub:type="footnote"[^>\n]*>\s*<p>.*?<\/p>\s*<\/div>/g)
        const b_notes = notes ? notes : []
        const pages = b.match(/<span[^>\n]+epub:type="pagebreak"[^>\n]+>/g)
        const b_pages = pages
          ? pages.map(pb => pb.replace(/^.*?title="([^"]+)".*?$/, '$1'))
          : []
        const b_page_count = b_pages.length
        const b_note_count = b_notes.length
        ...
        if (
          a.page_count < MAX_PAGES && a.note_count < MAX_NOTES &&
          a.page_count + b_page_count < MAX_PAGES && a.note_count + b_note_count < MAX_NOTES
        ) {
          const block_count =
          ...
          return {
            ...
            note_count: a.note_count + b_note_count,
            chunky: new_chunky,
          }
        } else {
          ...
          return {
            ...
            note_count: b_note_count,
            chunky: new_chunky,
          }
        }
      },
      { page_count: 0, note_count: 0, chunky: [{ start: '', end: '', blocks: [] }] }
    )
    ...
    return chunks.chunky.map((chunk, i) => {
      ...
      return {
        id: `${id}-${pad(i + 1, 2)}`,
        name:
          chunks.chunky.length > 1 && chunk.start !== '' && chunk.end !== ''
