...
const normHtml = html => {
  return html
    ...
    .replace(/<br>/gi, '<br />')
    .replace(/<video([^>]*?) controls ([^>]*?)>/gi, '<video$1 controls="controls" $2>')
...
const chunkify = ({ id, name, final_html }) => {
  ...
  if (page_count <= CHUNK_SIZE) {
    ...
    const chunks = block_html.reduce((a, b) => {
      const pagebreaks = b.match(/<span[^>\n]+epub:type="pagebreak"[^>\n]+>/g)
      const b_pages = pagebreaks ? pagebreaks.map(pb => pb.replace(/^.*?title="([^"]+)".*?$/, '$1')) : []
      ...
      if (a.page_count < CHUNK_SIZE && a.page_count + b_page_count < CHUNK_SIZE) {
        const block_count = last_chunk >= 0 ? a.chunky[last_chunk].blocks.length : 0
        const new_blocks = block_count > 0 ?
          a.chunky[last_chunk].blocks.slice(0, block_count - 1)
            .concat([a.chunky[last_chunk].blocks[block_count - 1].concat(b)]) : [b]

        const new_chunky = a.chunky
        new_chunky[last_chunk] =
          { start: a.chunky[last_chunk].start === '' ? b_first_page : a.chunky[last_chunk].start
          , end: b_last_page === '' ? a.chunky[last_chunk].end : b_last_page
          , blocks: new_blocks
          }

        return { page_count: a.page_count + b_page_count
               , chunky: new_chunky
               }
      } else {
        ...
        return { page_count: b_page_count
               , chunky: new_chunky
               }
      }
    ...
    return chunks.chunky.map((chunk, i) => {
      ...
      return { id: `${id}-${pad(i + 1, 2)}`
             , name: chunks.chunky.length > 1 ? `${name} (pp. ${chunk.start}-${chunk.end})` : name
