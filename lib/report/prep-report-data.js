...
const chunkify = ({ id, name, final_html }) => {
  ...
  const total = note_count + page_count
  ...
  if (total <= CHUNK_SIZE) {
    ...
    const chunks = block_html.reduce(
      (a, b) => {
        ...
        const b_total_count = b_page_count + b_notes.length
        ...
        if (
          a.total_count < CHUNK_SIZE &&
          a.total_count + b_total_count < CHUNK_SIZE
        ) {
          const block_count =
          ...
          return {
            total_count: a.total_count + b_total_count,
            chunky: new_chunky,
          }
        } else {
      ...
      { total_count: 0, chunky: [{ start: '', end: '', blocks: [] }] }
    )
