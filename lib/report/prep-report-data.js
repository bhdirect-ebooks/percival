  ...
  if (page_count <= CHUNK_SIZE) {
    ...
    const chunks = block_html.reduce(
      (a, b) => {
        ...
        // TODO filter out footnote matches before extracting first and last pages
    ...
    return chunks.chunky.map((chunk, i) => {
      ...
      return {
        id: `${id}-${pad(i + 1, 2)}`,
        name:
          // TODO instead of checking chunky.length, we need to actually check if chunk.start and chunk.end are not empty strings
