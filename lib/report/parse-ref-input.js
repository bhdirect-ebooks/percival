...
const checkRefsInHtml = html_string => {
  ...
  const regex = /(data-(?:context|ref))='({"scripture":[^}]+})'/gi
  ...
  for (const match of matches) {
    ...
    const markup_data = parseExplicit(ref_data.scripture, {
      vers: 'default',
      lang: 'en',
    })
    ...
    if (markup_data.length > 0) {
      ...
      html_string = html_string.replace(
        match.match,
        `${match.attr}='${JSON.stringify(ref_data)}'`
      )
    }
  }
  return html_string
...
const reparseWithContext = (
  ...
  const regex = /<a data-ref='({"scripture":[^}]+})'>([^<]+)<\/a>/gi
  ...
  // add context tag
  html = `<hr data-context='${JSON.stringify(ctxt_data)}' />\n${html}`.replace(
    /&quot;parsing&quot;:&quot;([^}]+)&quot;/g,
    '"parsing":"$1"'
  )
