...
const { parseExplicit, parseWithContext } = require('../parse')
...
const epub_dir = fs.readFileSync(
  path.join(report_loc, 'epub_dir.txt'),
  'utf-8'
)
const text = path.join(epub_dir, 'OEBPS', 'text')
...
app.use('/epub', express.static(text))
app.use('/data', bodyParser.json())
app.use('/parse', bodyParser.json())
app.use('/parseselection', bodyParser.json())
...
app.post('/data/blocks/:id', (req, res) => {
  ...
  if (req.query.hasOwnProperty('in') && req.query.in === 'html') {
    const current_refs = db
      .get('blocks')
      .get(req.params.id)
      .value().refs
    const parsed_html = checkRefsInHtml(req.body.html, current_refs)
    ...
    const current_html = current_block.html
      .replace(/"({[^}]+})"/g, "'$1'")
      .replace(/&quot;/g, '"')
    const current_refs = current_block.refs

    const parsed_html = reparseWithContext(
      input,
      current_html,
      current_refs,
      opts
    ...
    block = db
      ...
      .get(req.params.id)
      .assign({
        html: normHtml(html, req.params.id),
        refs,
      })
      .write()
  } else {
...
app.post('/parseselection', (req, res) => {
  const selection = req.body.selection
  const context = req.body.context ? req.body.context.trim() : null
  const opts = { vers: req.query.vers, lang: req.query.lang }

  const refDataArr =
    context && /^[\w .,;:-]+$/i.test(context)
      ? parseWithContext(selection, context, opts)
      : parseExplicit(selection, opts)

  const parsedText = refDataArr.length ? getFinalMarkup(refDataArr) : selection
