...
const getBlocks = (html) => {
  const regex = /(?:<(figcaption|[uod]l|aside|table|blockquote)[^>]*?>[\s\S]+?<\/\1>|<(p|h[123456])[^>]*?>[^\n]+<\/\2>)/g
...
const getBooksInContext = (html, opts) => {
  return getBookPatterns(opts)
    .map(book => {
      return new RegExp(`\b${book.regexp.source}\b`).test(html) ? book.osis : []
    })
    .reduce((a, b) => {
      return a.concat(b)
    })
...
const getRefContext = (html) => {
  const regex = /<a data-cross-ref='{"scripture":"([^"]+)"/g
  const matches = html.match(regex)
...
const improveRefTags = (block, block_context, opts) => {
  ...
  if (ref_tags) {
    for (const tag of ref_tags) {
      ...
      if (!no_conf_prop && ref_obj.confidence > assignConfWithString(tagged_string, ref_obj.confidence)) {
        has_words = false
      }
      ...
      if (no_conf_prop || low_conf_val) {
        ...
        if (alt_len === 0 && low_conf_val) {
          ...
          if (ref_obj.hasOwnProperty('context')) delete ref_obj.context
          ref_obj.possible = uniq(alternates)
            .filter(alt => {
              return ref_obj.scripture.includes(alt)
            })

        }

        block = !has_words ?
          block.replace(tag, `<a data-cross-ref='${JSON.stringify(ref_obj)}'>${tagged_string}</a>`) :
          block.replace(tag, `${removed_text}<a data-cross-ref='${JSON.stringify(ref_obj)}'>${tagged_string}</a>`)
      }
    }
  }
...
const identifyAlternatives = (html, opts) => {
  ...
  for (const block of blocks) {
    ...
    const block_context = uniq(provided_context.concat(prev_block_context, prosaic_context, ref_context))

    html = html.replace(block, improveRefTags(block, block_context, opts))
    prev_block_context = uniq(prosaic_context.concat(ref_context))
  }
