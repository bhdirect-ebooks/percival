...
const getValidAlternates = (context_arr, ref, opts) => {
  return context_arr
    .map(context => {
      ...
      console.log(`  Context Array: ${context_arr}\n  Ref: ${ref}\n  RefData: ${ref_data}\n`)
      if (ref_data.length > 0) {
        const last = ref_data[ref_data.length - 1]
        if (last.ref_data.validity.valid) {
          return [ last.default ]
        }
      }
      return []
...
const improveRefTags = (block, block_context, opts) => {
  ...
  if (ref_tags && ref_tags.length > 0) {
    for (const tag of ref_tags) {
      let tagged_string = tag.replace(regex, '$1')
      const has_words = /^[^\d]+([^\d]+)?\d/i.test(tagged_string)
      let removed_text = ''

      if (has_words) {
        tagged_string = tagged_string.replace(/^[^\d]+([^\d]+)?(\d)/i, '$2')
        removed_text = tagged_string.replace(/^([^\d]+([^\d]+)?)\d.*?$/i, '$1')
      }
