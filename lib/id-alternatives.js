...
const books = require('./book-regexps')
...
const getBooksInContext = (html) => {
  return books
    .map(book => {
      return book.regexp.test(html) ? book.osis : []
    })
    .reduce((a, b) => {
      return a.concat(b)
    })
...
const improveRefTags = (block, block_context, opts) => {
  // TODO: find and handle groups of orphans with mixed validity, when invalid is just
  // verse, i.e., <[valid tag]>...</[valid]>(?=(?:,|-) ?)<[invalid]>\d+</[invalid]>
