...
const getProvidedContext = (html, opts) => {
  const regex = /data-(?:context|parsing)="([^"]+)"/g
    ...
    return matches
      ...
      .map(value => {
        if (/^\D+$/.test(value)) {
          ...
          return parse_data.refs[0].validity.valid
            ? parse_data.refs[0].default
            : parse_data.refs[0].validity.start.b
        }
      })
  } else {
    return []
...
const improveRefTags = (block, block_context, opts) => {
  ...
  if (ref_tags) {
    for (const tag of ref_tags) {
      ...
      const trustworthy =
        ref_obj.confidence ===
        assignConfWithString(tagged_string, ref_obj.confidence)
      ...
      if (no_conf_prop || low_conf_val) {
        const alternates = getValidAlternates(
          block_context,
          tagged_string,
          opts
        ).filter(alt => {
        ...
        if (alt_len === 0 && low_conf_val && trustworthy) {
          ...
          if (ref_obj.hasOwnProperty('message')) {
            const [head, ...tail] = alternates
            delete ref_obj.message
            if (ref_obj.hasOwnProperty('context')) delete ref_obj.context
            ref_obj.confidence = 8
            ref_obj.valid = true
            ref_obj.scripture = head
            ref_obj.possible = tail
          } else {
            if (ref_obj.hasOwnProperty('context')) delete ref_obj.context
            ref_obj.possible = alternates
          }
        ...
        block = !has_words
          ? block.replace(
              tag,
              `<a data-ref='${JSON.stringify(ref_obj)}'>${tagged_string}</a>`
            )
          : block.replace(
              tag,
              `${removed_text}<a data-ref='${JSON.stringify(
                ref_obj
              )}'>${tagged_string}</a>`
            )
        ...
        block = block.replace(
          tag,
          `<a data-ref='${JSON.stringify(
            ref_obj
          )}'>${removed_text}${tagged_string}</a>`
        )
      }
    }
  }
...
const identifyAlternatives = (html, opts) => {
  ...
  for (const block of blocks) {
    ...
    provided_context =
      block_provided.length > 0 ? block_provided : provided_context

    const prosaic_context = uniq(getBooksInContext(block))
    const ref_context = uniq(getRefContext(block))
    const block_context = uniq(
      provided_context.concat(prosaic_context, ref_context)
    )
...
const idAltInSingleBlock = (block, opts) => {
  ...
  const ref_context = uniq(getRefContext(block))
  const block_context = uniq(
    block_provided.concat(prosaic_context, ref_context)
  )
