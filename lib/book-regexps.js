...
const books = [
  ...
  {
    osis: ['Song'],
    regexp: /\b(?:S(?:S|gs?| of S|ngs?|o(?:[Sl]|ng(?: of S(?:o(?:ngs?|lom[oa]ns?)|alom[oa]ns?)|s(?: of S(?:o(?:ngs?|lom[oa]ns?)|alom[oa]ns?))?)?))|The Song(?: of S(?:o(?:ngs?|lom[oa]ns?)|alom[oa]ns?)|s of S(?:o(?:ngs?|lom[oa]ns?)|alom[oa]ns?)))\b(?! of (?:man|God))/,
  },
  {
    osis: ['Isa'],
...
const getFinalIndex = (regex, str) => {
  let m
  let final = 0

  while ((m = regex.exec(str)) !== null) {
    if (m.index === regex.lastIndex) regex.lastIndex++
    m.forEach((match, i, arr) => {
      if (i === arr.length - 1) final = regex.lastIndex
    })
  }
...
const getBooksInContext = html => {
  const obj_arr = books
    .map(book => {
      if (book.regexp.test(html)) {
        const global_b = new RegExp(book.regexp.source, 'g')
        const last_index = getFinalIndex(global_b, html)
  ...
  return sortBy(obj_arr, ['last_index']).reduce((a, b) => a.concat(b.osis), [])
}
