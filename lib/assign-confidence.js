...
const assignConfidence = (ref_data, before, base_conf, type) => {
  ...
  switch (type) {
  case 'explicit':
    if (/^(?:is\.?|kings?|mark|wisdom|numbers?|act|job|so(?:,|n[.,]?)?)\W/.test(found_text)) {
      return base_conf - 8
      ...
      const with_prev = before.slice(ref_data.indices[0 - 1], ref_data.indices[1])
      return /^[([]/.test(with_prev) ? base_conf - 6 : base_conf
    } else if (/^\d+$/.test(found_text) && /^[^.]+\.\d+\.1\b/.test(ref_data.osis)) {
      return base_conf - 6
    } else {
