...
const tagInParens = (html, opts) => {
  let data = []
  const paren_regex = /(?:<\/a>|<(?:p|li|td)[^>]*?>|[\])])(([^\n\r<>]+?)[([])([abcdefsv \d:;,.\\-\\-‒–—]+)(?:[)\]]|<a data-cross)/gi
  const paren_arr = html.match(paren_regex)

  if (paren_arr) {
    paren_arr.forEach(text => {
      const ref_data = parseAndReplaceParenStrings(text, paren_regex, opts)
