...
const tagRef = (ref_data, before, base_conf, type) => {
  ...
  const to_be_tagged = before.substring(ref_data.indices[0], ref_data.indices[1])

  if (type === 'explicit' && !is_valid && confidence < 10 || to_be_tagged.includes('<a')) {
    return before
    ...
    return before.splice(ref_data.indices[0], 0, `<a data-ref='${JSON.stringify(attr_value)}'>`)
  }
}
