...
const handleRangeErrors = html =>
  html.replace(
    /(<a data-ref='{"scripture":"","valid":false,[^}]*?}'>[^<]+)<\/a>([-‒–—-])<a[^>]*?>([^<]+<\/a>)/g,
    '$1$2$3'
  )
...
const reduceErrors = (html, opts) => {
  ...
  // html = html.replace(/<a data-ref='{"scripture":"[^"]+"[^}]*?}'> <\/a>/g, '')

  if (orphan_group_regex.test(html)) {
    const matches = html.match(orphan_group_regex)
    const b_regex = /"scripture":"([^.]+)\./g
    matches.forEach(group => {
      const group_books = uniq(
        group.match(b_regex).map(b_string => b_string.replace(b_regex, '$1'))
      )
      if (group_books.length > 1) {
        const err_cnts = group_books.map(b => {
          ...
          return ref_data.length > 0
            ? getErrCountsFromMarkup(getFinalMarkup(ref_data))
            : 99
        })
        const lowest_cnt = parseInt(getLowest(err_cnts))
        const best_index = err_cnts.indexOf(lowest_cnt)
        const untag_group = group.replace(/<a[^>]+>([^<]+)<\/a>/g, '$1')
        const ref_data = parseWithContext(
          untag_group,
          group_books[best_index],
          opts
        )
  ...
  return html
    .replace(/<\/a>(\w)/g, '</a> $1')
    .replace(/(data-ref[^>]+>[^<]*?)([\])])<\/a>/g, '$1</a>$2')
    // allow scripture tags within footnotes
    /*.replace(/<footer epub:type="footnotes">[\s\S]+<\/footer>/gi, match => {
      return match.replace(
        /<a data-ref='{"scripture":[^>]+>([^<]+)<\/a>/gi,
        '$1'
      )
    })*/
}
