...
const parseAndReplaceMatches = (regex, the_string, context, opts) => {
  ...
  for (let match of matches) {
    ...
    const matcher = new RegExp(`${escapeRegExp(match)}(?!</a>|\\W</a>|[\\-\\-‒–—]| ?[abcdef]\b| ?f+\b|[.:]?\\d| and \\d|, \\d{1,3}</a>)`)
    ...
    if (ref_data.length > 0) {
      ...
      ref_data.push({
        ...
        after_text: the_string.replace(matcher, `<a data-ref='{"scripture":"","message":"No context."}'>${match}</a>`)
      })
