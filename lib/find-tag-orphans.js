...
const getFinalMarkup = (ref_data_arr) => {
  return escapeRegExp(ref_data_arr[ref_data_arr.length - 1].after_text)
}

const orphan_regexps = [ // order matters!
  /\b((?:cf\.?|v(?:\.|er(?:\.|s(?:\.|es?)?)?|(?:[sv](?:\.|s\.?)?))?)([\d,; ]+)?\d+(?:[abcdef]|\.|)(?:[:\\-\\-‒–—]\d+(?:[abcdef]|)|f+|[:\\-\\-‒–—][\\-\\-‒–—\s;,\d:]+\d+)(?:[a\\-f]|\.|)([\d,;: ]+)?(?!["\\-\\-‒–—]|:\d))/gi,
...
const parseAndReplaceMatches = (regex, the_string, context, opts) => {
  ...
  for (let match of matches) {
    ...
    const matcher = new RegExp(`${escapeRegExp(match)}(?!\\W?</a>|[\\-\\-‒–—]| ?[abcdef]|:?\\d| and \\d|, \\d{1,3}</a>)`)
