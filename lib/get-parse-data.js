...
const getOptions = (verse_system, apoc) => {
  return {
    ...
    include_apocrypha: apoc,
    versification_system: verse_system,
  }
}

const parsedRefObj = (bcv_entities, bcv, context = '') => {
  return bcv_entities.map(ref_data => {
    ...
    return {
      ...
      osis: ref_data.osis,
      default: deep_data.valid.valid
        ? bcv.parse(full_ref).reversify('hcsb')
        : ref_data.osis,
...
const parsedGroupsObj = (bcv_group, bcv, context) => {
  return {
    indices: bcv_group.indices,
    refs: context
      ? parsedRefObj(bcv_group.entities, bcv, context)
      : parsedRefObj(bcv_group.entities, bcv),
  }
}

const getParseData = (text, opts = {}) => {
  const verse_system = opts.vers ? opts.vers : 'default'
  const with_context = !!opts.context
  const apoc = typeof opts.apoc !== 'undefined' ? opts.apoc : true
  const bcv = opts.lang ? initParser(opts.lang) : initParser('en')

  bcv.set_options(getOptions(verse_system, apoc))
  ...
  return parsed.map(group => {
    return opts.context
      ? parsedGroupsObj(group, bcv, opts.context)
      : parsedGroupsObj(group, bcv)
  })
}
