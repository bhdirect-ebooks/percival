...
const tagLocal = (html, opts) => {
  ...
  if (local_text_arr) {
    local_text_arr.forEach(text => {
      const cross_ref = JSON.parse(text.replace(/^<a data-ref='([^>]+)'>.*?$/, '$1'))
      const just_after = text.replace(/^<a data-ref=[^>]+"confidence":10}'>[^<]+<\/a>/i, '')
      ...
      if (!/^\W+$/.test(just_after)) {
        ...
        for (let i = 0; i < parts.length; i++) {
          if (!/</.test(parts[i])) {
            const ref_data = findAndTagOrphans(parts[i], cross_ref.scripture, opts)
